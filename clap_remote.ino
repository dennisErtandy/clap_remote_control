...
// mic attach to the A0 analog input



//using progmem to store in the Arduino flash memory instead of sram, since the IR data for air conditioner is to long
// IR DATA IS DIFFERENT FROM 1 REMOTE TO ANOTHER, record it first using IR sensor
...
int k = 0 ;
  //detect the first clap
if (stage == 0 && analogRead(A0)>MIC_THRESHOLD)
{
  delay(50);
  k=0;
  for(k=0;k<10;k++)
  {
    delay (10);
    //detect false input of clap
    if(analogRead(A0)<MIC_THRESHOLD)
    {
...
for(k=0;k<250;k++)
  {
    delay(2);
  //detect second clap
    if (stage == 1 && analogRead(A0)>MIC_THRESHOLD)
      {
        k=0;
        for(k =0;k<10;k++)
        {
